...
function agi_dial($extension, $callerId, $callerIdFull)
{
	...
	$query = "Select name ".
		 ...
		 "where ((ext_end is not null and extension <= $extension ".
		 "  and ext_end >= $extension) or ".
					...
					$max_replay=3)
{
	...
	if ($tempRes == 0)
		// pas de digit lue, on fait une premiÃ¨re pause
		// dans le but de ne pas faire entendre Ã  l'utilisateur le
		// prompt immÃ©diatement aprÃ¨s
	...
	while ($ndigits < $max)
	{
		...
		{
			// pas de touche appuyÃ©e au bout du timeout
			// --> relecture
